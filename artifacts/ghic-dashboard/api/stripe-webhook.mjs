...
async function readRawBody(req) {
  ...
  const chunks = [];
  // Read the stream first when anything is still there. A parsed `req.body`
  // is only fatal when the bytes are genuinely gone.
...
/**
 * Vercel parses a JSON request body into an object before the handler runs.
 * That is fatal here: the signature covers the bytes Stripe sent, and a
 * parsed-then-reserialized object is not those bytes. Turning the parser off
 * is what makes `readRawBody` above able to do its job.
 */
